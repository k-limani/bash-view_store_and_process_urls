...
function clean_up() {
	...
	# If there's a date without body text, it removes it
...
while true; do
    ...
    if [[ $user_input == ['q':-'Q'] ]]; then  # Exit program if "Q"
		...
		if [[ $is_date_added == false ]]; then
			insert_timestamp
			print_border "$BORDER_CHAR" >> $FILE_PATH # border
			is_date_added=true
		fi
